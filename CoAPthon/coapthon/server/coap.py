...
class CoAP(object):
    ...
    def notify(self, resource):
        ...
        # print resource.observe_client_list
        # print "[[[["
        ...
        for transaction in observers:
            with transaction:
                ...
                if ("introspection" not in resource.__dict__) \
                        or ("introspection" in resource.__dict__
                            and transaction.request.source in resource.observe_client_list):
                    print "OTL"
                    print "OTL"
                    print "OTL"
                    print "OTL"
                    ...
                    if "introspection" in resource.__dict__ \
                            and transaction.request.source in resource.observe_client_list:
                        resource.observe_client_list.remove(transaction.request.source)
                        print "OTT"
                        print "OTT"
                        print "OTT"
                        print "OTT"
                        print transaction.response.active
                        print transaction.response.active == str(False)
                        if transaction.response.active == str(False):
                            self._observeLayer.remove_subscriber(transaction.response)
                        transaction.response.type = defines.Types["NON"]

                        # option = Option()
                        # option.number = defines.OptionRegistry.BEARER.number
                        # option.value = transaction.request.bearer
