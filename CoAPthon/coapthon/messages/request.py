...
class Request(Message):
    ...
    def token_type_hint(self):
        ...
        value = None
        for option in self.options:
            if option.number == defines.OptionRegistry.TOKEN_TYPE_HINT.number:
                value = option.value
        return value
    ...
    def token_type_hint(self, token_type_hint):
        option = Option()
        option.number = defines.OptionRegistry.TOKEN_TYPE_HINT.number

        option.value = token_type_hint
        self.add_option(option)

    @token_type_hint.deleter
    def token_type_hint(self):
        self.del_option_by_number(defines.OptionRegistry.TOKEN_TYPE_HINT.number)
