...
class Scope(object):
    ...
    def __init__(self, available=None, default=None):
        ...
        print "scope scope scope scope scope scope scope scope scope scope scope scope scope scope scope scope scope scope scope scope scope scope scope scope scope scope scope scope scope scope "
...
class ScopeGrant(object):
    ...
    def _create_scope_handler(self):
        ...
        print "9999999999999999999999999999999999"
        print self.scopes
        print self.scope_class(available=self.scopes,
...
class AuthRequestMixin(object):
    ...
    def read_validate_params(self, request):
        ...
        Implicit Grant.
        """
        print request.get_param("client_id")
...
class AuthorizationCodeTokenHandler(AccessTokenMixin, GrantHandler):
    ...
    def process(self, request, response, environ):
        ...
        """
        #additional scopes.append method
        # if request.get_param("scope") is not None:
        #     self.scopes.append(request.get_param("scope"))
...
class AuthorizationCodeGrant(GrantHandlerFactory, ScopeGrant,
    ...
    def __call__(self, request, server):
        ...
        if ((request.method == "POST"
                ...
                and (request.path == server.token_path or request.path == server.id_path)):
        ...
        if ((request.method == "GET"
                ...
                and (request.path == server.authorize_path or request.path == server.id_path)):
            scope_handler = self._create_scope_handler()
            print scope_handler.available_scopes
...
class RefreshTokenHandler(GrantHandler):
    ...
    def process(self, request, response, environ):
        ...
        #refresh token scope setting
        if access_token.scopes:
            token_data["scope"] = encode_scopes(access_token.scopes)
